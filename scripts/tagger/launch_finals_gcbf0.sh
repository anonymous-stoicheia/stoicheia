#!/bin/bash
# Final 5x2 matrix REBASED on the gcb_fold_0 torso (decontaminated pretraining) —
...
cd "$TAGGER_ROOT"
source env.sh
TORSO=$GCB_DATA/runs/gcb_fold_0/best.pt
...
cfg["out_dir"] = f"$GCB_DATA/runs/tagger_gcbf0_f{fold}_s{seed}"
...
EOF
    ...
    sbatch --dependency=afterany:$J --time=01:00:00 scripts/slurm/tagger_eval.sbatch \
      "\$GCB_DATA/runs/tagger_gcbf0_f${FOLD}_s${SEED}" \
