...
#
# Usage: scripts/launch_chain.sh [n_jobs] [hours_per_job] [config]
#   scripts/launch_chain.sh 6 8 configs/greekcharbert.json    # 6 x 8h = 48h total budget
set -euo pipefail
N=${1:-6}
HOURS=${2:-8}
CONFIG=${3:-configs/greekcharbert.json}
cd "$(dirname "$0")/../.."
